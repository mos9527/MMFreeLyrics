...
public:
	...
	Config() {
		...
		fontSize = 35.0f;
		fontSizeImGui = 13.0f;
		fillColor = ImVec4(1, 1, 1, 0);
		strokeColor = ImVec4(0, 0, 0, 0);
...
#define VEC4_TO_RGB(name) float name ## R = name.x;float name ## G = name.y;float name ## B = name.z;
	bool Save() {
